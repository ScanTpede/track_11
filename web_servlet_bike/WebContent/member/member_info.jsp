			...
			<table class="boardForm">
			  ...
			  <tbody>
				...
				<tr>
				  <th>비빌번호</th>
				  <td>
				  	<c:forEach begin="1" end="${t_dto.getPassword_len() }">
				  		*
				  	</c:forEach>
				  </td>
				</tr>
				...
				<tr>
				  <th>취미</th>
					...
					<td>
					  <c:if test="${t_dto.getHobby_travel() eq 'y'}">  
					  	여행
					  </c:if>

					  <c:if test="${t_dto.getHobby_sports() eq'y'}">  
					  	운동
					  </c:if>

					  <c:if test="${t_dto.getHobby_reading() eq 'y'}">  
					  	독서
					  </c:if>
				 	</td>
