...
public class NoticeDao {
	...
	public ArrayList<NoticeDto> getNoticeListPage(String select,String search, int start, int end ){
		...
		String query ="select * from\r\n" + 
				...
				"from home_최호성_notice n, home_최호성_member m\r\n" + 
				"where n.reg_id = m.id\r\n" + 
				"and "+select+" like '%"+search+"%'  order by n.no desc) tbl)\r\n" + 
	...
	{
		int result = 0;
		String query = "delete home_최호성_notice\r\n" + 
				"where no = '"+no+"'";
		...
		try {
			...
			System.out.println("noticeDelete() 5류 :"+query);
			e.printStackTrace();
		}finally {
			DBConnection.closeDB(con, ps, rs);
		}
	...
	{
		int result = 0;
		String query = "update home_최호성_notice\r\n" + 
		...
		try {
			...
			System.out.println("noticeUpdate() 5류 :"+query);
			e.printStackTrace();
		}finally {
			DBConnection.closeDB(con, ps, rs);
		}
	...
	public NoticeDto getNextNotice(String no)
	{
		...
		String query = "select a.no, b.title\r\n" + 
				"from \r\n" + 
				"(select min(no) as no\r\n" + 
				"from home_최호성_notice\r\n" + 
				"where no > '"+no+"') a, home_최호성_notice b\r\n" + 
				"where a.no = b.no";
	...
	public int noticeSave(NoticeDto dto){
		int result = 0;
//		String query="insert into home_최호성_notice\r\n" + 
